...
| 层 | 文件 | 改动类型 |
|---|---|---|
| 框架手写 | `Unity/Assets/Scripts/Hotfix/Framework/Config/IDataTable.cs` | 改 |
| 框架手写 | `Unity/Assets/Scripts/Hotfix/Framework/Config/BaseDataTable.cs` | 改 |
| 框架手写 | `Unity/Assets/Scripts/Hotfix/HotfixLauncher.cs` | 改 |
| 模板（工具版·JSON） | `Config/Tools/Luban/Templates/cs-simple-json/table.sbn` | 改 |
| 模板（工具版·JSON） | `Config/Tools/Luban/Templates/cs-simple-json/tables.sbn` | 改 |
| 模板（工具版·二进制） | `Config/Tools/Luban/Templates/cs-bin/table.sbn` | 改 |
| 模板（工具版·二进制） | `Config/Tools/Luban/Templates/cs-bin/tables.sbn` | 改 |
...
- `cs-l10n-key` 模板（纯同步生成 `LanguageKey.cs` 静态 key 常量，无任何 `Task`/`LoadAsync`）。
- `cs-bin`/`cs-simple-json` 里的 `bean.sbn`（`PostInit` 无 async，不含 `Task`）。
- `cs-dotnet-json` / `cs-dotnet-bin` 模板（server 端 `${FuFramework.Config}`，本次不碰）。
- 源码版模板 `Config/luban/src/.../Templates/`（命名空间对不上，见 §2）。
- server 端生成产物 `Server/FuFramework.Config/`。

## 5. 模板改动明细

> 模板改动原则：**只把异步链从原生 `Task` 换成 `UniTask`，其余（命名空间、字段名、deserialize 逻辑、`PostResolveRef`/`PostInit` 钩子、`SetTranslateText`/`ResolveRef` 等）一律保持原样**，不引入任何行为变化。
