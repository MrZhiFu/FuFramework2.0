...
无前缀 + **ALL_CAPS**（下划线分隔），不限可见性：`public const string BAG = "Bag";`、`private const string HOTFIX_DLL_NAME = "Game.Hotfix";`

### 1.7 字段

按字段种类使用不同前缀（与 Rider 命名规则、根目录 `.editorconfig` 一致）：

| 字段种类                                   | 规则                     | 示例                              |
| ------------------------------------------ | ------------------------ | --------------------------------- |
| 私有实例字段（含 `readonly`、Unity 序列化） | `m_` + camelCase         | `m_fsmDict`、`m_itemDic`          |
| 非私有实例字段                             | `m_` + PascalCase        | `m_TableManager`、`m_CachedBytes` |
| 私有静态字段                               | `s_` + camelCase         | `s_instance`                      |
| 非私有静态字段                             | `s_` + PascalCase        | `s_Config`                        |
| 私有静态只读字段                           | `sr_` + camelCase        | `sr_gameVersion`                  |
| 非私有静态只读字段                         | `sr_` + PascalCase       | `sr_AssetBundle`                  |
| 常量（不限可见性）                         | 无前缀 + ALL_CAPS        | `MAX_COUNT`、`HOTFIX_DLL_NAME`    |

* 自动生成代码（.Gen.cs）不限制
* 存量代码仍为旧风格（`m_` + PascalCase），后续用 Rider 批量清理迁移，新代码按上表执行
...
`<summary>` 标签必须换行，不允许写成 `/// <summary>text</summary>` 的单行形式。每个类、方法、属性、字段、枚举成员都有完整的 `<summary>` 注释，包括私有方法：
...
| **Gen 文件字段无前缀**     | FairyGUI`CSharpAPIGen` 自动生成的 `.Gen.cs` 文件中 UI 控件字段使用无前缀的匈牙利风格命名（`btnLogin`、`txtUsername`），与手写代码的字段前缀（`m_`/`s_`/`sr_`）不同。修改生成器模板维护成本高，接受此差异。 |
...
```
命名：     PascalCase 类/方法/属性; camelCase 局部/参数; 字段 m_/s_/sr_ 前缀（私有 camelCase、非私有 PascalCase）; 常量 ALL_CAPS
