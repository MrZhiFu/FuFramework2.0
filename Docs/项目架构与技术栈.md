...
- **unity-cli**：已配置 bridge 包，可通过命令行或 Claude Code 操控 Unity Editor。新电脑克隆项目后双击 `Tools/UnityCli/install-unity-cli.bat` 即可安装 CLI 工具。详见 `Tools/UnityCli/README.md`。
...
### 模块系统（核心模式）
每个框架功能都是一个 **`ModuleBase`**（纯 C# 抽象类，由 `ModuleManager` 反射创建并驱动，不依赖 `MonoBehaviour`）。模块注册全部集中在 `HotfixLauncher.MainAsync()` 中，由 `ModuleManager` 统一管理生命周期（OnInit → OnUpdate/OnLateUpdate/OnFixedUpdate → OnDispose）。注册顺序不可随意修改——基础模块（ReferencePool、ObjectPool、FSM、Procedure、Event、Mono、Timer）必须先于功能模块（Asset、Download、Network、UI 等）注册。`ModuleManager` 与 `ModuleBase` 均在 Hotfix 程序集中。帧更新由 Hotfix 侧 `GameDriven`（MonoSingleton，位于 `Hotfix/Framework/Core/`）持有委托并自驱动 MonoBehaviour Update/LateUpdate/FixedUpdate，HotfixLauncher 在注册模块后挂接 ModuleManager 生命周期方法，AOT 无关。
...
### 游戏流程：Procedure 系统
基于 FSM 模块构建。`ProcedureBase` 子类定义游戏的各个阶段（启动、闪屏、登录、主菜单、战斗等）。生命周期：`OnInit`、`OnEnter`、`OnUpdate`、`OnLeave`。流程切换通过 `ChangeState<T>()` 实现。`ProcedureModule` 通过反射自动扫描所有 `ProcedureBase` 子类。
...
- **程序集定义**：使用 `.asmdef` 文件。AOT 侧仅含 `AOT`、`FuFramework.Core.Runtime`、`FuFramework.ModuleSetting.Runtime`；框架模块本体全部在 `Hotfix` 程序集中，不再拆分 Runtime/Editor 子程序集。Editor 工具集中在 `Assets/Editor/` 下，归 `Unity.Editor` 程序集。
