...
### 模块系统（核心模式）
每个框架功能都是一个 **`ModuleBase`**（继承自 `MonoBehaviour`，但生命周期由框架驱动，继承 MonoBehaviour 仅为了在 Inspector 面板中方便展示模块信息）。模块按顺序注册（参见 `Launcher.Modules.cs`），由 `ModuleManager` 统一管理。注册顺序不可随意修改——基础模块（ReferencePool、ObjectPool、FSM、Procedure、Event、Coroutine、Mono、Timer）必须先于功能模块（Asset、Download、Network、UI 等）注册。
