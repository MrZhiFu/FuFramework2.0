...
namespace FuFramework.Core.Hotfix.Agent;

/// <summary>
/// 基础组件代理类，用于管理组件与Actor之间的交互
/// </summary>
/// <typeparam name="TComponent">具体的组件类型</typeparam>
public abstract class BaseComponentAgent<TComponent> : IComponentAgent, IWorker where TComponent : BaseComponent
{
	/// <summary>
	/// 所有者的组件实例
	/// </summary>
	public TComponent OwnerComponent => (TComponent)Owner;

	/// <summary>
	/// 所有者的Actor实例
	/// </summary>
	public IActor Actor => Owner.Actor;
	...
	/// 所有者的类型
	/// </summary>
	public ushort OwnerType => Actor.Type;
	...
	public long Delay<T>(DateTime time, GameEventArgs eventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Delay<T>(ActorId, time - DateTime.Now, eventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long Delay<T>(long time, GameEventArgs eventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Delay<T>(ActorId, new DateTime(time) - DateTime.Now, eventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long Delay<T>(TimeSpan delay, GameEventArgs eventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Delay<T>(ActorId, delay, eventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long Schedule<T>(TimeSpan delay, TimeSpan interval, GameEventArgs gameEventArgs = null, int repeatCount = -1, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Schedule<T>(ActorId, delay, interval, gameEventArgs, repeatCount);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long Daily<T>(int hour = 0, int minute = 0, GameEventArgs gameEventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Daily<T>(ActorId, hour, minute, gameEventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long Weekly<T>(DayOfWeek dayOfWeek, int hour = 0, int minute = 0, GameEventArgs gameEventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Weekly<T>(ActorId, dayOfWeek, hour, minute, gameEventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	{
		long num = QuartzTimer.WithDayOfWeeks<T>(ActorId, hour, minute, gameEventArgs, dayOfWeeks);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long Monthly<T>(int dayOfMonth, int hour = 0, int minute = 0, GameEventArgs gameEventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.Monthly<T>(ActorId, dayOfMonth, hour, minute, gameEventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
	...
	/// <typeparam name="T">实现了ITimerHandler接口的定时任务处理器类型</typeparam>
	/// <returns>新创建的定时任务ID，可用于后续取消该任务</returns>
	public long WithCronExpression<T>(string cronExpression, GameEventArgs gameEventArgs = null, long unScheduleId = 0L) where T : ITimerHandler
	{
		Unscheduled(unScheduleId);
		long num = QuartzTimer.WithCronExpression<T>(ActorId, cronExpression, gameEventArgs);
		ScheduleIdSet.Add(num);
		return num;
	}
}
