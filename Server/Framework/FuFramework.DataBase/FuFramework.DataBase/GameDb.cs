...
namespace FuFramework.DataBase;
...
public static class GameDb
{
	...
	/// <exception cref="T:System.ArgumentNullException">当  ConnectionString 或 Name 为null时抛出</exception>
	/// <returns>返回数据库是否初始化成功</returns>
	public static async Task<bool> Init<T>(DbOptions dbOptions) where T : IDatabaseService, new()
	{
		ArgumentNullException.ThrowIfNull(dbOptions, "dbOptions");
	...
	/// <exception cref="T:System.InvalidCastException">当类型转换失败时抛出</exception>
	public static T As<T>() where T : IDatabaseService
	{
		ArgumentNullException.ThrowIfNull(_dbServiceImplementation, "_dbServiceImplementation");
	...
	{
		ArgumentNullException.ThrowIfNull(_dbServiceImplementation, "_dbServiceImplementation");
		return await _dbServiceImplementation.DeleteListAsync(filter);
	}
