...
/// 内部消息
/// </summary>
public sealed class InnerNetworkMessage : IInnerNetworkMessage, IMessage
{
	...
	public string ToFormatMessageString()
	{
		try
		{
			...
			handler.AppendLiteral("](");
			handler.AppendFormatted(MessageIdUtility.GetMainId(Header.MessageId).ToString().CenterAlignedText(3));
			handler.AppendLiteral(",");
			handler.AppendFormatted(MessageIdUtility.GetSubId(Header.MessageId).ToString().CenterAlignedText(3));
			handler.AppendLiteral(")");
			...
			handler.AppendFormatted(DeserializeMessageObject().ToJsonString());
	...
	{
		InnerNetworkMessage innerNetworkMessage = new InnerNetworkMessage();
		...
		innerNetworkMessage.SetMessageType(message.GetType());
		byte[] messageData = ProtoBufSerializerHelper.Serialize(message);
		innerNetworkMessage.SetMessageData(messageData);
		innerNetworkMessage.SetMessageHeader(messageObjectHeader);
		return innerNetworkMessage;
	}
	...
	public static InnerNetworkMessage Create(INetworkMessageHeader messageObjectHeader, byte[] messageData, Type messageType)
	{
		...
		innerNetworkMessage.SetMessageHeader(messageObjectHeader);
		innerNetworkMessage.SetMessageData(messageData);
		innerNetworkMessage.SetMessageType(messageType);
		return innerNetworkMessage;
	}
}
