...
internal class ListDecorator : ProtoDecoratorBase
{
	...
	{
		if (returnList && ImmutableCollectionDecorator.IdentifyImmutable(model, declaredType, out MethodInfo builderFactory, out PropertyInfo isEmpty, out PropertyInfo length, out MethodInfo methodInfo, out MethodInfo addRange, out MethodInfo finish))
		{
	...
	internal static MethodInfo GetEnumeratorInfo(TypeModel model, Type expectedType, Type itemType, out MethodInfo moveNext, out MethodInfo current)
	{
		...
		if (type2 != null)
		{
			type2 = type2.MakeGenericType(itemType);
			type = type2;
		}
