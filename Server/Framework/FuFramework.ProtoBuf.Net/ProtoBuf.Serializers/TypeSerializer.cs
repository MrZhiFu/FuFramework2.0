...
internal sealed class TypeSerializer : IProtoTypeSerializer, IProtoSerializer
{
	...
	private IProtoSerializer GetMoreSpecificSerializer(object value)
	{
		...
		for (int i = 0; i < serializers.Length; i++)
		{
			...
			if (protoSerializer.ExpectedType != ExpectedType && Helpers.IsAssignableFrom(protoSerializer.ExpectedType, type))
			{
	...
	public void Write(object value, ProtoWriter dest)
	{
		...
		{
			IProtoSerializer protoSerializer = serializers[i];
			if (protoSerializer.ExpectedType == ExpectedType)
			{
	...
	public object Read(object value, ProtoReader source)
	{
		...
		while ((num3 = source.ReadFieldHeader()) > 0)
		{
			...
			for (int i = num2; i < fieldNumbers.Length; i++)
			{
				...
				else if (expectedType != ExpectedType && ((IProtoTypeSerializer)protoSerializer).CanCreateInstance() && expectedType.IsSubclassOf(value.GetType()))
				{
