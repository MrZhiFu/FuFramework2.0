...
public abstract class ClientSession : IClientSession, IGameAppSession, IBufferSetter
{
	...
	public ValueTask SendAsync(byte[] data, CancellationToken cancellationToken = default(CancellationToken))
	{
		return SendAsync(new ArraySegment<byte>(data), cancellationToken);
	}

	public ValueTask SendAsync(ReadOnlyMemory<byte> data, CancellationToken cancellationToken = default(CancellationToken))
	{
