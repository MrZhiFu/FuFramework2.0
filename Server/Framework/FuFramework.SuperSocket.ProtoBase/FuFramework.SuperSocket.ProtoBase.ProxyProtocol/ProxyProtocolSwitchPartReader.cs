...
internal class ProxyProtocolSwitchPartReader<TPackageInfo> : ProxyProtocolPackagePartReader<TPackageInfo>
{
	private const int SWITCH_PART_SIZE = 12;

	private static readonly byte[] PROXYPROTOCOL_V2_SIGNATURE = new byte[12]
	{
	...
	public override bool Process(TPackageInfo package, object filterContext, ref SequenceReader<byte> reader, out IPackagePartReader<TPackageInfo> nextPartReader, out bool needMoreData)
	{
		...
		needMoreData = false;
		if (reader.IsNext(PROXYPROTOCOL_V2_SIGNATURE, advancePast: true))
		{
			nextPartReader = ProxyProtocolPackagePartReader<TPackageInfo>.ProxyProtocolV2Reader;
			return false;
		}
		int read = 0;
		try
		{
			...
			{
				nextPartReader = ProxyProtocolPackagePartReader<TPackageInfo>.ProxyProtocolV1Reader;
				return false;
			}
			return true;
		}
	...
	{
		read = 0;
		if (!reader.IsNext(PROXY_TAG, advancePast: true))
		{
			return false;
		}
