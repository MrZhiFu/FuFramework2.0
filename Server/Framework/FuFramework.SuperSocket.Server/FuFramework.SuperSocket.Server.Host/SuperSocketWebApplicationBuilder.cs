...
public class SuperSocketWebApplicationBuilder : IHostApplicationBuilder
{
	...
	/// </summary>
	public IHostBuilder Host => _hostApplicationBuilder.GetType().GetProperty("Host", BindingFlags.Instance | BindingFlags.Public)?.GetValue(_hostApplicationBuilder) as IHostBuilder;

	/// <summary>
	/// Initializes a new instance of the <see cref="T:FuFramework.SuperSocket.Server.Host.SuperSocketWebApplicationBuilder" /> class.
	/// </summary>
	...
	public IHost Build()
	{
		IHost host = _hostApplicationBuilder.GetType().GetMethod("Build")?.Invoke(_hostApplicationBuilder, Array.Empty<object>()) as IHost;
