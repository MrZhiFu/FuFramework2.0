...
public abstract class PackageHandlingSchedulerBase<TPackageInfo> : IPackageHandlingScheduler<TPackageInfo>
{
	...
	{
		IPackageHandler<TPackageInfo> packageHandler = PackageHandler;
		Func<IAppSession, PackageHandlingException<TPackageInfo>, ValueTask<bool>> errorHandler = ErrorHandler;
		try
		{
		...
		{
			if (await errorHandler(session, new PackageHandlingException<TPackageInfo>("Session " + session.SessionID + " got an error when handle a package.", package, e)))
			{
