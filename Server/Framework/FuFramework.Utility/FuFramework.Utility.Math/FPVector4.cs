...
public struct FPVector4
{
	...
	{
		if (!(obj is FPVector4 fPVector))
		{
			return false;
		}
		if (x == fPVector.x && y == fPVector.y && z == fPVector.z)
		{
			return w == fPVector.w;
		}
		return false;
	...
	{
		FPVector4 result = default(FPVector4);
	...
	{
		FP fP = value.x * value.x + value.y * value.y + value.z * value.z + value.w * value.w;
		FP fP2 = FP.One / FP.Sqrt(fP);
		result.x = value.x * fP2;
		result.y = value.y * fP2;
		result.z = value.z * fP2;
		result.w = value.w * fP2;
	}
