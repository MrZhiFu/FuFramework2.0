...
/// <summary>
/// 提供对 <see cref="T:System.String" /> 类型的扩展方法。
/// </summary>
public static class StringExtension
{
	...
	public static string CenterAlignedText(this string text, int width)
	{
		...
		return new string(' ', count) + text + new string(' ', count);
	}
	...
	public static string WordWrap(this string text, int width)
	{
		...
		{
			if ((text2 + text3).Length > width)
			{
				NewSentence.AppendLine(text2);
				text2 = "";
			}
			text2 = text2 + text3 + " ";
		}
	...
	public static string RemoveSuffix(this string self, string toRemove)
	{
		if (!self.IsNullOrEmpty())
		{
			...
			return self.Substring(0, self.Length - toRemove.Length);
		}
		return self;
	}
	...
	/// <exception cref="T:System.ArgumentNullException">当字符串为 null 或空字符串时抛出。</exception>
	...
	/// <param name="value">要验证的字符串。</param>
	/// <param name="name">参数名称，用于异常消息。</param>
	/// <exception cref="T:System.ArgumentNullException">当字符串为 null、空字符串或仅包含空白字符时抛出。</exception>
	...
	public static int[][] SplitTo2IntArray(this string str, char sep1 = ';', char sep2 = '+')
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			array2[i] = array[i].SplitToIntArray(sep2);
		}
		return array2;
	}
	...
	public static void CreateAsDirectory(this string path, bool isFile = false)
	{
		...
		{
			path.CreateAsDirectory(isFile: true);
	...
	/// <exception cref="T:System.ArgumentNullException">当前字符串为null时抛出。</exception>
	public static bool EqualsFast(this string self, string target)
	{
		...
		int num2 = target.Length - 1;
		while (num >= 0 && num2 >= 0 && self[num] == target[num2])
		{
			num--;
			num2--;
		}
	...
	public static bool StartsWithFast(this string self, string target)
	{
		...
		int num = 0;
		int num2 = 0;
		while (num < length && num2 < length2 && self[num] == target[num2])
		{
			num++;
			num2++;
		}
