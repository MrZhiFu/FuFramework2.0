...
public static class TimeHelper
{
	...
	{
		return timestampSeconds * 10000000 + 621355968000000000L;
	}
	...
	{
		return timestampMillisSeconds * 10000 + 621355968000000000L;
	}
	...
	{
		return MillisecondsTimeStampToDateTime(UnixTimeMilliseconds(), utc: true) - MillisecondsTimeStampToDateTime(timestamp, utc: true);
	}
	...
	{
		return DateTime.Now - MillisecondsTimeStampToDateTime(timestamp, utc: true);
	}
	...
	{
		return GetCrossDays(TimestampToDateTime(beginTimestamp), hour);
	}
	...
	{
		DateTime startTime = UtcToUtcDateTime(beginTimestamp);
		DateTime endTime = UtcToUtcDateTime(afterTimestamp);
	...
	{
		int num = (int)(endTime.Date - startTime.Date).TotalDays;
	...
	{
		DateTime startTime = UtcToLocalDateTime(startTimestamp);
		DateTime endTime = UtcToLocalDateTime(endTimestamp);
	...
	public static bool IsSameWeek(DateTime start, DateTime end)
	{
		...
		return start.AddDays(7 - num).Date >= end.Date;
	}
	...
	public static DateTime GetDayOfWeekTime(DateTime dateTime, DayOfWeek day)
	{
		...
		return dateTime.AddDays(num - num2).Date;
	}
	...
	{
		DateTime time = UtcToUtcDateTime(timestamp1);
		DateTime time2 = UtcToUtcDateTime(timestamp2);
	...
	{
		return DateTime.Today.AddDays(1.0).AddSeconds(-1.0);
	}
	...
	{
		return GetWeekStartTime().AddDays(7.0).AddSeconds(-1.0);
	}
	...
	{
		return GetMonthStartTime().AddMonths(1).AddSeconds(-1.0);
	}
	...
	{
		return GetYearStartTime().AddYears(1).AddSeconds(-1.0);
	}
	...
	{
		return DateTime.Today.AddDays(2.0).AddSeconds(-1.0);
	}
	...
	{
		return GetWeekStartTime().AddDays(7.0);
	}
	...
	{
		return GetNextWeekStartTime().AddDays(7.0).AddSeconds(-1.0);
	}
	...
	{
		return GetNextMonthStartTime().AddMonths(1).AddSeconds(-1.0);
	}
	...
	{
		return GetStartTimeOfWeek(date).AddDays(7.0).AddSeconds(-1.0);
	}
	...
	{
		return GetStartTimeOfMonth(date).AddMonths(1).AddSeconds(-1.0);
	}
	...
	{
		return GetStartTimeOfYear(date).AddYears(1).AddSeconds(-1.0);
	}
	...
	{
		DateTime time = UtcToLocalDateTime(timestamp1);
		DateTime time2 = UtcToLocalDateTime(timestamp2);
		return IsSameDay(time, time2);
	}
	...
	{
		if (time1.Date.Year == time2.Date.Year && time1.Date.Month == time2.Date.Month)
		{
			return time1.Date.Day == time2.Date.Day;
		}
		return false;
	}
}
