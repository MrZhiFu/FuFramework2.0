...
{
    /// <summary>
    /// 红点模块配置-RedPointSetting 创建器
    /// </summary>
    public static class RedPointSettingCreator
    {
        private const string AssetName = "RedPointSetting.asset";                          // 资源名称
        ...
        [MenuItem("FuFramework/框架模块配置/红点模块配置/创建", false, 1)]
        public static void CreateDefaultAssetSetting()
        {
            // 检查资源是否已存在
            var existingSetting = AssetDatabase.LoadAssetAtPath<RedPointSetting>(FullPath);
            if (existingSetting)
            {
                EditorUtility.DisplayDialog("创建失败", "RedPointSetting 资源已存在, 请勿重复创建!", "确定");
            ...
            var assetSetting = ScriptableObject.CreateInstance<RedPointSetting>();
        ...
        [MenuItem("FuFramework/框架模块配置/红点模块配置/查找", false, 2)]
        public static void FindAssetSetting()
        {
            var guids = AssetDatabase.FindAssets("t:RedPointSetting");
            if (guids.Length > 0)
            {
                var path         = AssetDatabase.GUIDToAssetPath(guids[0]);
                var assetSetting = AssetDatabase.LoadAssetAtPath<RedPointSetting>(path);
                Selection.activeObject = assetSetting;
                EditorUtility.FocusProjectWindow();
            ...
            {
                var createNew = EditorUtility.DisplayDialog("未找到资源", "未找到任何 RedPointSetting 资源，是否创建新的？", "创建", "取消");
