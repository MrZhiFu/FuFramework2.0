...
namespace FuFramework.ObjectPool.Editor
{
    ...
    internal sealed class ObjectPoolComponentInspector : GameComponentInspector
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var poolComponent = target as ObjectPoolComponent;
            if (!poolComponent) return;

            if (IsPrefabInHierarchy(poolComponent.gameObject))
            {
                EditorGUILayout.LabelField("对象池总个数：", poolComponent.Count.ToString());
