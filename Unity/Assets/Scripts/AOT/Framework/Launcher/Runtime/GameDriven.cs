...
namespace FuFramework.Launcher.Runtime
{
    ...
    public class GameDriven : MonoSingleton<GameDriven>
    {
        ...
        public void RestartGame()
        {
            ...
            BootstrapProcess.RunAsync(EnterHotfixAsync).Forget();
        }

        /// <summary>
        /// 热更入口回调。
        /// 由 BootstrapProcess 在加载完 Hotfix 程序集后调用，反射进入 Hotfix.HotfixLauncher.MainAsync。
        /// </summary>
        /// <param name="view">AOT 加载界面句柄，透传给热更入口用于收尾关闭。</param>
        internal async UniTask EnterHotfixAsync(IBootstrapView view)
        {
            var hotfixAssembly = GetHotfixAssembly();
            if (hotfixAssembly == null)
            {
                FuLogger.LogError("[GameDriven] 未找到已加载的 Hotfix 程序集，无法进入热更入口。");
                return;
            }
            ...
            if (mainMethod == null)
            {
                FuLogger.LogError("[GameDriven] 未找到热更入口 Hotfix.HotfixLauncher.MainAsync。");
                return;
            }
