        ...
        private async UniTaskVoid GetAssetPackageVersionInfo()
        {
            ...
            try
            {
                ...
                if (httpJsonResult.Code > 0)
                {
                    ...
                    FuLogger.LogError($"获取资源版本信息异常=> Req:{Utility.Json.ToJson(jsonParams)} Resp:{rstJson}");
            ...
            {
                FuLogger.LogError($"获取资源版本信息异常=>Error:{e.Message}   Req:{Utility.Json.ToJson(jsonParams)}");
                LauncherUIHelper.SetTipText("获取资源版本信息异常, 正在重试...");
                await UniTask.WaitForSeconds(3);
                GetAssetPackageVersionInfo().Forget();
            }
