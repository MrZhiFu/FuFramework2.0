...
AOT（Ahead-of-Time）是游戏随包体预编译的代码，负责**极简启动流程**：显示加载界面 → 初始化资源包 → 检查更新 → 下载资源 → 加载 Hotfix 程序集 → 进入热更。

基于 HybridCLR，AOT 仅保留必需的启动代码、`GameSetting` 全局配置和少量基础工具（日志、路径、JSON 等），所有框架模块和游戏逻辑均在 Hotfix 中。

## 2. 目录结构

```
AOT/
├── Launcher.cs              # AOT 入口 MonoBehaviour（挂载到首个场景 GameObject，启动流程）
...
`Launcher` 是实现 `ILaunchView` 的 MonoBehaviour，在 `Start()` 中调用 `LaunchProcess.RunAsync(this)` 启动流程。同时负责 `DontDestroyOnLoad` 保证 `GameSetting` 跨场景存活。
