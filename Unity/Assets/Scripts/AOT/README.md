...
`Launcher` 是实现 `IBootstrapView` 的 MonoBehaviour，在 `Start()` 中调用 `BootstrapProcess.RunAsync(this)` 启动引导流程。同时负责 `DontDestroyOnLoad` 保证 `GameSetting` 跨场景存活。

## 5. 引导流程

详见 [Bootstrap README](Bootstrap/README.md)。
