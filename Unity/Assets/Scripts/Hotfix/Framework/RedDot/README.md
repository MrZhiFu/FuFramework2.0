...
FuFramework RedDot 模块是游戏框架的红点提示系统，用于管理 UI 中的红点（未读标记）层级结构和计数。该模块采用 **Pull 模式（Calculator）** 驱动，业务模块注册计算函数，框架通过 EventModule 事件触发 + OnUpdate 批处理自动重算，最后通过 EventModule 批量广播变更通知 UI 刷新。
...
- **RawCount**：节点自身的原始计数（由 Calculator 计算）
...
// 邮件数据变更时广播事件，触发所有监听该事件的 Calculator 重算
...
1. **Calculator 优先**：叶子节点使用 `Register` 注册计算函数，框架自动管理重算时机
...
Play 模式下通过菜单 `FuFramework > 调试 > 红点调试面板` 打开。

**功能：**
- **树形 / 平铺视图**：按父子层级展示所有红点节点，或扁平列出
- **搜索过滤**：按 Key 名实时搜索，父节点若子节点匹配会保留显示
- **自动刷新**：每 0.5 秒重绘一次，实时反映 Calculator 计算结果
- **节点信息**：显示 Key、静态/动态标记、最终计数、原始计数、聚合逻辑、显示模式、清理策略、已读/脏/Calculator 标记
- **操作按钮**：静态节点可点击"已读"调用 `MarkRead`，"刷新"调用 `GetState` 触发一次取值
- **全部展开 / 全部折叠**：一键展开或折叠所有父节点

> 调试面板通过反射访问 Hotfix 程序集中的 `RedDotModule` 和 `RedDotNode`，无需 Editor程序集直接引用 Hotfix。
