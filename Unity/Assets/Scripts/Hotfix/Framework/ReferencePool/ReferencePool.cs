...
namespace FuFramework.ReferencePool.Runtime
{
    ...
    public static partial class ReferencePool
    {
        ...
        /// </summary>
        private static readonly Dictionary<Type, ReferenceCollection> ReferenceCollectionDict = new();
        ...
        public static void ClearAll()
        {
            ...
            {
                foreach (var (_, refCollection) in ReferenceCollectionDict)
                {
        ...
        private static void _CheckReferenceType(Type refType)
        {
            ...
            if (refType == null)
                throw new InvalidOperationException("[ReferencePool] 引用类型为空.");

            if (!refType.IsClass || refType.IsAbstract)
                throw new InvalidOperationException("[ReferencePool] 引用类型不是非抽象类类型.");

            if (!typeof(IReference).IsAssignableFrom(refType))
                throw new InvalidOperationException($"[ReferencePool] 引用类型 '{refType.FullName}' 不是 IReference 类型.");
        }
        ...
        private static ReferenceCollection _GetReferenceCollection(Type refType)
        {
            ...
            lock (ReferenceCollectionDict)
            {
                if (ReferenceCollectionDict.TryGetValue(refType, out referenceCollection)) return referenceCollection;
                referenceCollection = new ReferenceCollection(refType);
                ReferenceCollectionDict.Add(refType, referenceCollection);
            }
