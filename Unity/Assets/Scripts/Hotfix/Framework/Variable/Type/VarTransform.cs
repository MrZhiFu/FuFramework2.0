using Hotfix.Framework.Core;
using UnityEngine;

// ReSharper disable once CheckNamespace
namespace Hotfix.Framework.Variable
{
    /// <summary>
    /// 自定义 Transform 变量类。
    /// 功能：
    ///     1. 可以像正常 Transform 变量一样使用，且底层使用引用池优化了内存。
    /// </summary>
    public sealed class VarTransform : GenericVariable<Transform>
    {
        /// <summary>
        /// 初始化 VarTransform 变量类的新实例。
        /// </summary>
        public VarTransform() { }

        /// <summary>
        /// 从 UnityEngine.Transform 到 VarTransform 变量类的隐式转换。
        /// </summary>
        /// <param name="value">值。</param>
        public static implicit operator VarTransform(Transform value)
        {
            var varValue = GlobalModule.ReferencePoolModule.Acquire<VarTransform>();
            varValue.Value = value;
            return varValue;
        }

        /// <summary>
        /// 从 VarTransform 变量类到 UnityEngine.Transform 的隐式转换。
        /// </summary>
        /// <param name="value">值。</param>
        public static implicit operator Transform(VarTransform value) => value.Value;
    }
}
